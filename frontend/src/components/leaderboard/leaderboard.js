...
class Leaderboard extends React.Component {
    ...
    render(){
       ...
       if (this.props.users){
        ...
        //    })
        leaderboard=
        <div>{sorted.map((user,idx) => {
            let leaderboardInfo = idx === 0 ? "leaderboard-info-top1" : idx === 1 ? "leaderboard-info-top2" : idx === 2 ? "leaderboard-info-top3" : "leaderboard-info1"
                    {n += 1}
                    return <div key={idx} className={leaderboardInfo} style={{display:'flex', 
                    // justifyContent:'space-between', 
                    marginTop: '5px'}}>
                                <div className="rankers">
                                    { idx === 0 ? <RiVipCrownFill className="crown"/> :
                                        idx===1 ? <RiVipCrownLine className="crown"/> : 
                                            idx === 2 ? <CgCrown className="crown"/> : ""}
